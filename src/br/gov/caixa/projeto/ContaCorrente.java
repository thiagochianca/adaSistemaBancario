...
public class ContaCorrente {
    ...
    public boolean transferenciaValores(ContaCorrente contaDepositado, double valorTransferencia) {
        if (this.contaCliente.getClassificacaoCliente()=="PF") {
            if (valorTransferencia > 0 && this.consultaSaldo() > valorTransferencia) {
                ...
                criaHistorico("Transferencia", valorTransferencia, this.consultaSaldo(), this.contaCliente.getIdCliente(),
                        contaDepositado.contaCliente.getIdCliente(), "Transferencia realizada com sucesso");
                return true;
            } else {
                criaHistorico("Transferencia", valorTransferencia, this.consultaSaldo(), this.contaCliente.getIdCliente(),
                        contaDepositado.contaCliente.getIdCliente(), "Transferencia não realizada");
                return false;
            }
        } else if (this.contaCliente.getClassificacaoCliente()=="PJ") {
            if (valorTransferencia > 0 && this.consultaSaldo() > valorTransferencia) {
    ...
    public boolean depositoConta(double valorDeposito) {
        if (valorDeposito > 0) {
            ...
            criaHistorico("Deposito", valorDeposito, this.saldoConta, this.contaCliente.getIdCliente(), this.contaCliente.getIdCliente(), "Deposito realizado com sucesso");
            return true;
        }
        criaHistorico("Deposito", valorDeposito, this.saldoConta, this.contaCliente.getIdCliente(), this.contaCliente.getIdCliente(), "Deposito não foi realizado");
        return false;
    ...
    public boolean saqueConta(double valorSaque) {
        if (this.contaCliente.getClassificacaoCliente()=="PF") {
            if (valorSaque <= this.saldoConta) {
                ...
                criaHistorico("Saque", valorSaque, this.saldoConta, this.contaCliente.getIdCliente(), this.contaCliente.getIdCliente(), "Saque realizado com sucesso");
                return true;
            }
            criaHistorico("Saque", valorSaque, this.saldoConta, this.contaCliente.getIdCliente(), this.contaCliente.getIdCliente(), "Saque não foi realizado");
            return false;
        } else if (this.contaCliente.getClassificacaoCliente()=="PJ") {
            if (valorSaque <= this.saldoConta) {
